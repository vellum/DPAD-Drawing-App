...
- (instancetype)initWithRect:(CGRect)rect
{
	if (( self = [super init] ))
	{
		...
		NSLog(@"dpad / initwithrect: %@", NSStringFromCGRect(rect));

		self.joystickRadius = CGRectGetWidth(rect) / 2;
		self.thumbRadius = self.joystickRadius; // 32.0f;
		self.deadRadius = 0.0f;
		...
		stick.fillColor = [UIColor colorWithWhite:0.2f alpha:1.0f];
		stick.strokeColor = [UIColor clearColor];
		stick.lineWidth = 0.0f;
		stick.path = CGPathCreateWithEllipseInRect(rect, NULL);
		stick.antialiased = NO;
		targetPos = stick.position;

		SKShapeNode *yourline = [SKShapeNode node];
		CGMutablePathRef pathToDraw = CGPathCreateMutable();
		CGFloat pad = 15.0f;
		CGFloat w = rect.size.width;
		CGPathMoveToPoint(pathToDraw, NULL, 0 + pad, 0 + pad);
		CGPathAddLineToPoint(pathToDraw, NULL, w - pad, w - pad);

		CGPathMoveToPoint(pathToDraw, NULL, w   - pad, 0 + pad);
		CGPathAddLineToPoint(pathToDraw, NULL, 0 + pad, w - pad);
		yourline.path = pathToDraw;
		[yourline setLineWidth:0.5f];
		[yourline setStrokeColor:[UIColor colorWithWhite:1.0f alpha:0.5f]];
		yourline.antialiased = NO;
		[stick addChild:yourline];
...
{
	/*
	 * if (CGPointEqualToPoint(CGPointZero, stick.position)) {
	 *  return;
	 * }
	 */
	CGFloat curx, cury, tx, ty, dx, dy;

	curx = stick.position.x;
	cury = stick.position.y;
	tx = targetPos.x;
	ty = targetPos.y;
	dx = tx - curx;
	dy = ty - cury;
	...
	curx += dx * m;
	cury += dy * m;
	stick.position = CGPointMake(curx, cury);
}
...
- (void)updateVelocity:(CGPoint)point
{
	...
	{
		dx = cosAngle * dist; // _joystickRadius;
		dy = sinAngle * dist; // _joystickRadius;
	}
...
- (void)forceTouchBegin:(NSSet *)touches withEvent:(UIEvent *)event
{
